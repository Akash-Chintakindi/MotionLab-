...
describe("Lesson 10 quiz content", () => {
  it("targets the right lesson", () => {
    expect(quiz10.lessonId).toBe("lesson-10-oscillations");
  });

  it("has 8–10 questions with unique ids", () => {
    expect(quiz10.questions.length).toBeGreaterThanOrEqual(8);
    expect(quiz10.questions.length).toBeLessThanOrEqual(10);
    const ids = quiz10.questions.map((q) => q.id);
    expect(new Set(ids).size).toBe(ids.length);
  });

  it("mixes conceptual and calculation questions", () => {
    const cats = quiz10.questions.map((q) => q.category);
    expect(cats).toContain("conceptual");
    expect(cats).toContain("calculation");
  });

  it("every question is well-formed and has an explanation", () => {
    for (const q of quiz10.questions) {
